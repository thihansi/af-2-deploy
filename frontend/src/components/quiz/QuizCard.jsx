...
const QuizCard = ({ question, onAnswer, showNext }) => {
  ...
  return (
    <motion.div 
      ...
      className="bg-indigo-900/40 backdrop-blur-md rounded-xl p-6 shadow-lg border-2 border-indigo-500/30 relative overflow-hidden"
    >
      ...
      <div className="mb-6 text-center relative z-10">
        <div className="inline-block px-4 py-1 rounded-full bg-indigo-700/50 border border-indigo-500/30 text-yellow-300 text-sm font-medium mb-3">
          {question.type === 'flag' ? '🚩 Flag Challenge' : 
           question.type === 'capital' ? '🏙️ Capital City Quiz' : 
           question.type === 'region' ? '🗺️ Region Explorer' : '🌍 Country Quest'}
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {displayOptions.map((option, index) => {
          ...
          let optionClass = "p-4 rounded-xl cursor-pointer transition-all duration-300 text-center ";
          
          if (showFeedback) {
            if (option === question.correctAnswer) {
              optionClass += "bg-gradient-to-r from-green-500/70 to-green-600/70 text-white border-2 border-green-400 shadow-md shadow-green-500/20";
            } else if (option === selectedOption) {
              optionClass += "bg-gradient-to-r from-red-500/70 to-red-600/70 text-white border-2 border-red-400 shadow-md shadow-red-500/20";
            } else {
              optionClass += "bg-indigo-800/30 text-gray-400 border border-indigo-700/30";
            }
          } else {
            optionClass += "bg-indigo-800/50 text-white hover:bg-indigo-700/70 border-2 border-indigo-600/30 hover:border-indigo-500/50 shadow-md hover:shadow-lg hover:shadow-indigo-500/10";
          }
      ...
      {showFeedback && (
        ...
        >
          <div className={`text-xl font-bold mb-2 ${isCorrect ? 'text-green-400' : 'text-red-400'} flex items-center justify-center gap-2`}>
            {isCorrect ? (
              <>
